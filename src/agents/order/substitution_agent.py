...
class SubstitutionAgentPlugin:
    ...
    async def get_substitutes(
        self, skus_to_check: Annotated[list[str], "List of SKU IDs to find substitutes for"]
    ) -> Annotated[dict[str, str], "A dictionary mapping each SKU to its substitute SKU"]:
        """
        ...
        for sku in skus_to_check:
            if (
                ...
                availability = await self.check_inventory_availability([f"{available_skus_dict[sku]['substitute']}:0"])
                substitutes[sku] = {"substitute_sku": available_skus_dict[sku]["substitute"], "available_quantity": availability}                        
                logger.info(f"Found substitutes for SKUs: {substitutes} with availability: {availability}")
