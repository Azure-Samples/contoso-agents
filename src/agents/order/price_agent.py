...
class PricingAgentPlugin:
    ...
    ) -> Annotated[str, "Information about the applicable discount percentage or lack thereof"]:
        """
    ...
    async def calculate_final_price(
        ...
        pricesheet = await self.data_store.get_data(customer_id, "customer")
        if pricesheet and "pricesheet" in pricesheet and "items" in pricesheet["pricesheet"]:
            for price in pricesheet["pricesheet"]["items"]:
                if price["sku"] == sku:
                    ...
                    result["pricing_details"].append(f"Customer-specific price applied: ${price['price']}")
        
        # Check for quantity discount
        discount_info = await self.data_store.get_data(sku, "discount")
        if discount_info and "minimum" in discount_info and quantity >= discount_info["minimum"]:
            discount_percentage = discount_info["discount"] * 100
            discounted_price = result["final_unit_price"] * (1 - discount_info["discount"])
        ...
        if not result["has_customer_specific_price"] and not result["has_quantity_discount"]:
